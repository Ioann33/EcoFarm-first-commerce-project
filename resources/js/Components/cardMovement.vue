...
<!--                            <span class="font-300" style="margin-left: 15px">{{ movement.amount }} <sup>{{ movement.unit }}</sup></span>-->
                        ...
                        <h4 class="pt-1 font-600">{{ movement.amount }} <sup>{{ movement.unit }}</sup></h4>
                        ...
                        <div class="mt-3">
                            Отгрузка
                                <span v-if="dir=='in'">из</span>
                                <span v-else>в</span> <br>
                            <i class="fa-fw select-all fas "></i> {{ movement.storage_name }}
                        </div>
                    </div>
...
<!--                        <a href="#" v-if="canCancel"        @click.prevent="setOrderStatus(movement.order_id, 'canceled')" class="btn shadow-bg shadow-bg-m btn-m btn-full mb-3 rounded-s text-uppercase font-900 shadow-s bg-red-dark">Отменить</a>-->
                            ...
                            <a v-if="this.status=== 'opened' && movement.link_id!==null"  @click='this.$emit("showIngredients", movement.link_id)'  class="btn btn-m btn-full mb-3 rounded-0 text-uppercase font-900 shadow-s bg-yellow-dark"> Состав </a>
...
<div v-if="this.dir !== 'out'">
    <div v-if="movement.goods_type === 2"> <!-- Установить цену для ГотовойПродукции -->
                            <router-link v-if="editPrice" :to="{name: 'SetPriceAndPull', params: {movement_id: movement.id}}">
                                <a class="btn btn-m btn-full mb-3 rounded-0 text-uppercase font-900 shadow-s bg-red-light">
                                    Установить цену оприходовать</a>
                            </router-link>
    ...
    <div v-else>
                            ...
                            <a v-if="editPrice"            @click='this.$emit("getMovementId", movement.id)' data-menu="menu-setPrice" class="btn shadow-bg shadow-bg-m btn-m btn-full mb-3 rounded-s text-uppercase font-900 shadow-s bg-green-dark">
                                Установить цену и оприходовать</a>
                            <a href="#" v-else-if="this.dir==='in'" @click='this.$emit("getMovementId", movement.id)'                           class="btn shadow-bg shadow-bg-m btn-m btn-full mb-3 rounded-s text-uppercase font-900 shadow-s bg-green-dark">
                                Оприходовать</a>
